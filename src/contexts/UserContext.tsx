...
export const UserContextProvider = ({ children }: { children: ReactNode }) => {
    ...
    const setAccessToken = (newAT: string) => {
        ...
        authenticatedAxiosInstance.defaults.headers.authorization = "Bearer " + newAT;
    };
            ...
            async (error) => {
                ...
                if (error.response.status === 401 && refreshToken) {
                    ...
                    const { accessToken: newAccessToken } =
                        (await authApi.authControllerRefresh({ refreshToken })).data;
    ...
    const [storeApi,] = useState(
        () => {
            return new StoreApi(new Configuration({
                basePath: process.env.REACT_APP_API_BASE_URL
            }), undefined, authenticatedAxiosInstance);
        }
    )
